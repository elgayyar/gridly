...
describe('Starting tests for gridly', function() {
  ...
  it('website title should be gridly', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('gridly');
    })
  });

  it('network-name should be gridly@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('gridly@0.0.1.bna');
    });
  });

  it('navbar-brand should be gridly',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('gridly');
    });
  });
